...
export function firebaseErrorMessage(error: unknown) {
  ...
  if (code.includes("admin/configuration")) return "Configure a credencial do Firebase Admin nas variáveis protegidas da Vercel.";
  if (code.includes("admin/internal")) return "O backend administrativo não conseguiu concluir a operação. Confira os registros da Vercel.";
...
export function observeFirebaseAuth(callback: (user: FirebaseUserSummary | null) => void): Unsubscribe {
  const { auth } = services();
  return onAuthStateChanged(auth, (user) => callback(summarizeUser(user)));
}
...
async function callAdmin<TOutput>(input?: object): Promise<TOutput> {
  ...
  try {
    ...
    const error = new Error("Não foi possível acessar o backend administrativo da Vercel.") as Error & { code?: string };
