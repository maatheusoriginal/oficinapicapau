...
function permissionValues(data: unknown, role: UserRole, employeeId: string): UserPermission[] {
  ...
  return data.filter((item): item is UserPermission => permissions.includes(item));
}

async function callerFrom(request: NextRequest) {
...
function responseError(error: unknown) {
  ...
  if (code.includes("email-already-exists")) {
    return NextResponse.json({ error: { code: "already-exists", message: "Este e-mail já está cadastrado." } }, { status: 409 });
  }
  ...
  return NextResponse.json({ error: {
    ...
    message: configuration ? error.message : "Não foi possível concluir a operação administrativa.",
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { db } = firebaseAdmin();
    const [authenticationUsers, profiles] = await Promise.all([listAllAuthenticationUsers(), db.collection("userAccess").get()]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json() as UserInput & { action?: unknown };
    ...
    if (action === "create") {
      ...
      try {
        ...
        return NextResponse.json({ data: { user: publicUser(created.uid, { name, email, phone, role, employeeId, active, permissions: userPermissions }, created, true) } });
      } catch (error) {
    ...
    if (action === "update") {
      ...
      return NextResponse.json({ data: { user: publicUser(uid, { name, email, phone, role, employeeId, active, permissions: userPermissions }, authUser, true) } });
    }
